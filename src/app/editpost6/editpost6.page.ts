...
@Component({
  selector: 'app-editpost6',
  templateUrl: './editpost6.page.html',
  styleUrls: ['./editpost6.page.scss'],
})
export class Editpost6Page implements OnInit {
  categoryId : any;
  categoryName : any;
  selectedVideo = false;
  ...
  async getPostById(){
    ...
    this.postService.getPostById(postData).subscribe((result) => {
      loading.dismiss();
      this.selectDate = result.weight_selectDate;
      this.age = result.weight_age;
      this.weight = result.weight_log;
      this.height = result.height_log;
      this.comment = result.weight_comment;
  ...
  async registerNewPost() {
    ...
    this.postService.updatePost6(this.formData).subscribe((result) => {
