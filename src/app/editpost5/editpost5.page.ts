...
@Component({
  selector: 'app-editpost5',
  templateUrl: './editpost5.page.html',
  styleUrls: ['./editpost5.page.scss'],
})
export class Editpost5Page implements OnInit {
  categoryId : any;
  categoryName : any;
  selectedVideo = false;
  ...
  async getPostById(){
    ...
    this.postService.getPostById(postData).subscribe((result) => {
      loading.dismiss();
      this.selectDate = result.things_selectDate;
      this.comment = result.things_comment;
  ...
  async registerNewPost() {
    ...
    this.postService.updatePost5(this.formData).subscribe((result) => {
