...
@Component({
  selector: 'app-editpost2',
  templateUrl: './editpost2.page.html',
  styleUrls: ['./editpost2.page.scss'],
})
export class Editpost2Page implements OnInit {
  categoryId : any;
  categoryName : any;
  selectedVideo = false;
  ...
  async getPostById(){
    ...
    this.postService.getPostById(postData).subscribe((result) => {
      ...
      // this.postlist = result.services;
        ...
        console.log("this.post_content", this.post_content);
        this.post_name = result.post_name;
        this.post_age = result.post_age;
        this.post_relation = result.post_relation;
        this.post_diagnosis = result.post_diagnosis
  ...
  async registerNewPost() {
    this.formData.append('post_name', this.post_name);
    this.formData.append('post_age', this.post_age);
    this.formData.append('post_relation', this.post_relation);
    this.formData.append('post_diagnosis', this.post_diagnosis);
    ...
    this.postService.updatePost2(this.formData).subscribe((result) => {
  ...
  async submit(){
    if(this.post_name == undefined || this.post_age == undefined || this.post_relation ==undefined || this.post_diagnosis ==undefined){
      this.presentToast('Please enter post data correctly.');
      return;
    }
